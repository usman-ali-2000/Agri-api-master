...
app.get("/login", async (req, res) => {
  ...
  try {
    const user1 = await uploadLogin.find().sort({ _id: -1 });
...
app.post('/dailyentry/filter', async (req, res) => {
  try {
    const { date } = req.body; // expecting string in "DD/MM/YYYY"
    if (!date) return res.status(400).json({ error: 'Date is required' });

    console.log(`Filtering daily entries for date: ${date}`);

    // Simple string match
    const entries = await DailyEntry.find({ date: date });
...
app.post('/dailyentry/filter-range', async (req, res) => {

  try {
    const { startDate, endDate, farmId } = req.body; // "16/2/2025"
    ...
    // Query using $expr to convert string date to Date
